...
export function MediaLibraryPage() {
  ...
  const totalBytes = useMemo(() => mediaAssets.reduce((sum, asset) => sum + asset.size, 0), [mediaAssets]);
    ...
    <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 p-4 md:p-8">
      <section className="overflow-hidden rounded-3xl border border-border/70 bg-gradient-to-r from-card via-card to-muted/20 shadow-sm">
        <div className="grid gap-6 p-6 lg:grid-cols-[1.1fr_0.9fr] lg:p-8">
          <div className="space-y-4">
            <div className="inline-flex items-center gap-2 rounded-full border border-border/70 bg-background/70 px-3 py-1 text-xs font-medium uppercase tracking-[0.2em] text-muted-foreground">
          ...
          <Card className="border-border/70 bg-background/70 shadow-none">
            <CardHeader className="space-y-1">
              <CardTitle className="text-lg font-semibold">Upload new media</CardTitle>
              <CardDescription>Choose a file and store it in your S3 bucket.</CardDescription>
            </CardHeader>
      ...
      <section className="space-y-4">
        <div>
          ...
          <h3 className="mt-1 text-2xl font-semibold tracking-tight">Uploaded assets</h3>
        </div>

        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {mediaAssets.map((asset) => (
            <Card key={asset._id} className="overflow-hidden border-border/70 bg-card/90 shadow-sm">
